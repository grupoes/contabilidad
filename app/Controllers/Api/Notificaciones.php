...
class Notificaciones extends ResourceController
{
    ...
    public function getMontosPdtRenta()
    {
        ...
        $pdts = $pdtRenta->query("SELECT pr.id_pdt_renta, pr.ruc_empresa, pr.periodo, pr.anio, pr.total_compras, pr.total_ventas, ap.id_archivos_pdt, ap.nombre_pdt FROM pdt_renta pr INNER JOIN archivos_pdt0621 ap ON ap.id_pdt_renta = pr.id_pdt_renta WHERE pr.estado = 1 AND ap.estado = 1 AND pr.anio = 11 and pr.periodo between 1 and 6 /*AND pr.total_ventas = 0*/ order by pr.periodo asc")->getResultArray();
