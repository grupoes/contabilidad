...
class Mensajes extends BaseController
{
    ...
    public function mensajesAllId($id)
    {
        ...
        $consulta = $envio->select("id, message, DATE_FORMAT(fecha_envio, '%d-%m-%Y %H:%i:%s') as fecha_envio, numero_whatsapp, razon_social, ruc, estado")->where('mensaje_id', $id)->orderBy("FIELD(estado, 'no enviado', 'pendiente', 'enviado')", '', false)->findAll();
