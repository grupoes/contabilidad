  ...
  fetch(url_base + "reporte-detallado", {
    ...
    .then((data) => {
      ...
      data.forEach((v, i) => {
        ...
        const esNotaCredito = v.origen === "nota_credito";
        const sign = esNotaCredito ? "-" : "";

        const valorVenta =
          parseFloat(v.total_icbper) > 0
        ...
        html += `<tr>
                    ...
                    <td>${sign}${v.total_exonerado ?? "0.00"}</td>
                    <td>${sign}${v.total_gravado ?? "0.00"}</td>
                    <td>${sign}${v.total_inafecto ?? "0.00"}</td>
                    <td>${sign}${valorVenta}</td>
                    <td>${sign}${valorVenta}</td>
                    <td>${sign}${v.total_igv ?? "0.00"}</td>
                    <td>0.00</td>
                    <td>${sign}${v.total_icbper ?? "0.00"}</td>
                    <td>${sign}${v.total ?? "0.00"}</td>
                    <td>1</td>
                    <td>${glosa}</td>
                    <td>${cuenta}</td>
                    <td>${afectacion}</td>
                    <td>HABIDO</td>
                    <td>ACTIVO</td>
                    <td>${v.estado === "f" ? "ANULADO" : v.homologacion_estado}</td>
                    <td>${v.referencia ?? ""}</td>
