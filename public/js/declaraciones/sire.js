...
function descargarArchivos(id) {
    ...
    fetch(base_url + "contribuyentes/getId/" + id)
        .then((res) => res.json())
        .then((data) => {
            titleModalDownload.innerHTML = `DESCARGAR ARCHIVOS SIRE - <span class="text-primary" id="nameComp">${data.razon_social}</span>`;
        });
}
...
formRectificacion.addEventListener("submit", (e) => {
    ...
    fetch(base_url + "rectificar-sire", {
        method: "POST",
        body: formData,
    })
        .then((res) => res.json())
...
function viewArchivosSire(e, sire_id) {
    e.preventDefault();

    const archivos_sire = document.getElementById("archivos_sire");

    fetch(`${base_url}sire/files/${sire_id}`)
        .then((res) => res.json())
        .then((data) => {
            let html = "";
