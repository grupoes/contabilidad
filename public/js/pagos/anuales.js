...
function renderPagos(idcontribuyente) {
  fetch(base_url + "render-pagos-anuales/" + idcontribuyente)
    .then((res) => res.json())
    .then((data) => {
      viewPagos(data);
    });
...
formPago.addEventListener("submit", (e) => {
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
      ...
      messageSpinner.textContent = "Registrando pago Anual...";

      fetch(`${base_url}cobros/pagar-anual`, {
        method: "POST",
        body: formData,
      })
        .then((res) => res.json())
...
function getMontoPendiente() {
  ...
  fetch(`${base_url}monto-anual/${idContribuyente.value}`)
    .then((res) => res.json())
    .then((data) => {
      if (data.status == "success") {
        monto.value = data.monto;
